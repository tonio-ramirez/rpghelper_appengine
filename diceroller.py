...
dice_roll_matcher_regexp = re.compile(r'^([0-9]+)(\.5)?[dD]([0-9]+)([+-][0-9]+)?$')


def roll_dice(description, use, campaign, user_prefs, parent = None):
    global dice_roll_matcher_regexp
    if parent:
    ...
    m = dice_roll_matcher_regexp.match(new_roll.description)
    if m:
